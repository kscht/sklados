...
// Generic-фолбэк (лестница D-44, ступень 5).
export default function Things() {
  ...
  const { data: kinds = [] } = useQuery({
    queryKey: ["kinds"],
    queryFn: () => sql<{ kind: string; n: number }[]>(
      "SELECT kind, count() AS n FROM thing GROUP BY kind ORDER BY n DESC;",
    ),
  });
  const { data: rows = [] } = useQuery({
    queryKey: ["things", kindFilter],
    queryFn: () => sql<ThingRow[]>(
      kindFilter
  ...
  return (
    <main className="p-6">
      <h1 className="text-xl font-bold mb-4">
        <Link to="/" className="text-neutral-400 hover:underline">Домовой</Link> / things
      </h1>
      <div className="flex flex-wrap gap-1.5 mb-6">
        {kinds.map((k) => (
          <Link
            key={k.kind}
            to={`/things?kind=${k.kind}`}
          ...
          {rows.map((r, i) => (
            <tr key={(r.id as string) ?? i} className="border-b border-neutral-100 hover:bg-neutral-50">
              <td className="py-1.5 pr-4">
                <Link to={nodeHref(r.id)} className="text-blue-700 hover:underline">{cell(r.name)}</Link>
              </td>
