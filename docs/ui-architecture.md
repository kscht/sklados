...
> **Ревизия 2026-07-21** (после фазы 1, D-44…D-50) — см. раздел в конце: лестница разрешения раскладки, контракт-как-план-запроса, `ui_subtypes`, `EdgeTargetPicker`, actions v1, правила клик-экономии, workspace-as-data. Открытые вопросы 1, 4, 6 ниже — закрыты этой ревизией.
...
## Ревизия 2026-07-21 — уточнения после фазы 1 (D-44…D-50)

Вводные изменились: канон 41 ввёл `kind='view'`, категории стартуют пустыми (D-33), рёбра типизированы классами (D-43, D-51). Уточнения к модели выше:

### Лестница разрешения раскладки (D-44)

```
1. Личный view пользователя для kind        (kind='view', can_access {edit: self})
2. Семейный/общий view для kind             (kind='view', расшарен)
3. ui на концепте kind (+ ui_subtypes)      ← дефолт, 80% случаев
4. ui на классе (container/party/…)         ← опционально, наследование
5. Generic fallback: поле=строка, ребро=ссылка
```

Первое совпадение побеждает. `view` — одновременно «именованный экран» (умный список, дашборд, inbox) и «переопределение раскладки» (D-19).

### Контракт виджета = план запроса (D-45)

Контракт (`fields`, `edges: [{predicate, dir, fields}]`, `vocab`) — не только проверка применимости: движок собирает **union контрактов раскладки в один SELECT** (анти-N+1) + LIVE SELECT на узел для реактивности. Виджет без данных не рендерится (кроме `always`); отсутствующий в реестре виджет — generic-фолбэк с бейджем, не падение.

### `ui_subtypes` (D-46) и `EdgeTargetPicker` (D-47)

Per-subtype override merge'ится поверх базового `ui` на том же концепте (`event/appointment` ≠ `event/terminal_session`). Пикеры форм не хардкодятся: `EdgeTargetPicker(predicate)` читает `to_kinds` концепта ребра и разворачивает классы — «кому одолжить?» = `party` (человек, семья, прокат), «куда положить?» = `container` (полка, холодильник, NUC).

### Actions v1 (D-48, закрывает вопрос 4)

```yaml
actions:
  - slug: mark_done,       do: mutate,  sets: { status: 'done' }     # валидация — под-схемой статусов
  - slug: lend,            do: relate,  edge: lent_to                # диалог из типизации ребра
  - slug: run_maintenance, do: job,     template: template_to_bmw    # задел под плейбуки
```

### Клик-экономия (D-49)

Правила движка, не новые компоненты: `form_quick` (1–2 поля, остальное — дефолты и progressive disclosure); **контекст экрана = дефолты формы** («+» в полке проставляет `contains`); search-first (`/` → топ-результат → Enter); bulk и inline-edit — свойство `NodeList`, не привилегия inbox; action `clone` для «ещё такой же»; `ui.header` ≤ 5 виджетов. Клик-бюджеты как DoD фаз: добавить ≤ 3 действий, найти ≤ 2, переложить = 1 (dnd), одолжить ≤ 3, bulk-категоризация 25 шт ≤ 4.

### Workspace-as-data (D-50)

Вертикаль («Склад») — не захардкоженный раздел, а узел `kind='view', subtype='workspace'`: `sidebar` из view-ссылок, `home`-экран. Один маршрут `/w/[slug]` рендерит любой workspace; список вертикалей = запрос с `can_access`. В коде только шелл: логин, рендерер, реестр компонентов уровня 3 (`LocationTree`), generic-фолбэк при пустой базе. Workspace — **линза**; хранилища — `location`/`item`-контейнеры (D-51), перенос вещей — пересадка `contains` (D-52), любой view это просто показывает.

### Desktop-парадигма: главный вход — пространственный грид (D-53, D-54)

Главный вход приложения — **рабочий стол**: всё пространство браузера в иконках-узлах, как iOS-springboard. Механика целиком из существующих примитивов:

- **стол/папка** = `view(subtype='desktop')`; вложенность любая; перетащил иконку на иконку → создалась папка; workspace'ы — папки на столе;
- **размещение** = ребро `references` с `order` (упорядоченная сетка, как iOS — без свободных координат, переживает любую ширину экрана);
- **«симлинки» бесплатны**: узел один — размещений много (дерево хранения + папка «Ремонт» + закладки одновременно); удаление размещения ≠ удаление узла;
- **иконка** = `ui.graph.icon` концепта kind, переопределяется пер-узловым полем `icon` (emoji/слаг);
- **шаринг и личное**: папка — обычный узел (`can_access`); закладки = персональный desktop (D-19).

Таблицы, карточки, дерево, формы — не отменяются: это рабочие экраны **внутри** узлов; грид — навигационный слой над ними.

### Следствие для roadmap

Фаза 2 сразу строит `/w/[slug]` + системные view из seed (`ws_sklad`, inbox, список, дерево, «одолжено») вместо голого `/items`; фаза 4 — **inbox-first** (категорий на старте ноль, D-33), каталожные фильтры включаются по мере роста дерева.
