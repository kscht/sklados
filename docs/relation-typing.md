...
└── class_container  «может содержать» (D-51)
    ├── kind_location     ← комната, полка, подвал
    ├── kind_item         ← холодильник, сейф, чемодан
    └── kind_device       ← NUC с дисками, сервер с ВМ
```
...
| `contains` | `['container']` | NONE | содержать может место, вещь, устройство (D-51) |

### `contains` vs `part_of` (D-52)

- **`contains` = физическое местонахождение всегда** — включая «установлено в»: NUC contains SSD (`installed: true` на ребре), холодильник contains молоко, полка contains запасной SSD. Апгрейд/ремонт = та же пересадка ребра, что «переложить перфоратор» — единая история перемещений (since/until) не рвётся.
- **`part_of` = несъёмная структура** — багажник part_of Весты, полка part_of шкафа: то, что не «вынимают на полку».
- Критерий item-vs-location для контейнера: важна вещность (бренд, гарантия, поломка, продажа) → `item`; чистое адресуемое пространство → `location`. Пограничные (стеллаж) — оба легальны, дерево строится обходом рёбер и не ломается.
